...
def receive_messages(client_socket):
    ...
    try:
        ...
        while True:
            ...
            # ── File Transfer IP Resolution (Task 4) ───────────────────
            elif line.startswith("FILE_IP "):
                parts = line.split(' ', 2)
                if len(parts) >= 3:
                    filename = parts[1]
                    recipient_ip = parts[2]
                    print(f"\r[SYSTEM] Starting file transfer for '{filename}' to {recipient_ip}...")
                    # Trigger sender as internal module in background thread
                    threading.Thread(
                        target=file_sender.send_file,
                        args=(filename, recipient_ip),
                        daemon=True
                    ).start()
                print("> ", end="", flush=True)
...
def start_client():
    ...
    # ── Start automatic UDP receiver (Task 4) ──────────────────────────
    receiver_thread = threading.Thread(
        target=file_receiver.start_receiver,
        daemon=True
    )
    ...
    # ── Send JOIN ──────────────────────────────────────────────────────
    client_socket.sendall(f"JOIN {username} {email}\n".encode('utf-8'))
    ...
    try:
        while True:
            ...
            # ── /sendfile — UDP file transfer (Task 4) ────────────────
                ...
                if len(parts) < 3:
                    ...
                    filename  = parts[1]
                    recipient = parts[2]
                    msg = f"CMD_SENDFILE {filename} {recipient}\n"
                    client_socket.sendall(msg.encode('utf-8'))
