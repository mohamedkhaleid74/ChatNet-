...
def remove_client(username):
    ...
    """
    with clients_lock:
        sock = clients.pop(username, None)   # Remove safely; None if missing
    if sock:
        try:
            sock.close()
        except Exception:
...
def handle_client(client_socket, addr):
    ...
    try:
        ...
        username = first_line.strip()[5:].strip()   # Everything after "JOIN "

        if not username:
            send_to(client_socket, "400 Bad Request ; Username cannot be empty")
            client_socket.close()
            return
